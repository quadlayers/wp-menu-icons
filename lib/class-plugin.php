...
final class Plugin {
	...
	private function __construct() {
		...
		add_action(
			...
			function () {
				...
				$icons = apply_filters( 'wp_menu_icons_register_icons', self::$default_icons );
	...
	public static function selected_icons( $menu_id = null ) {
		...
		if ( empty( self::registered_icons()->{$selected_icons} ) ) {
			return false;
		}

		return self::registered_icons()->{$selected_icons};
	}
	...
	public function get_default_icons() {
		...
		$register_icons = array(
			...
			'fontawesome'  => array(
				...
				'url'     => plugins_url( 'assets/frontend/icons/fontawesome/css/font-awesome.min.css', WPMI_PLUGIN_FILE ),
			...
			'foundation'   => array(
				...
				'url'     => plugins_url( 'assets/frontend/icons/foundation/foundation-icons.min.css', WPMI_PLUGIN_FILE ),
			...
			'themify'      => array(
				'url'     => plugins_url( 'assets/frontend/icons/themify/themify-icons.min.css', WPMI_PLUGIN_FILE ),
				'name'    => 'Themify Icons',
	...
	public function nav_menu_item_icons() {

		$nav_menu_item_icons = array();

		foreach ( wp_get_nav_menu_items( self::nav_menu_selected_id() ) as $key => $item ) {
